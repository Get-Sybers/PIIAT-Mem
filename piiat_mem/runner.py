...
# (Order preserved; the two dfir_* plugins are the custom ones in ./plugins.)
TIMELINE_PLUGINS = [
    "dfir_processes.DfirProcesses",   # process create times (psscan; unlinked too)
    ...
    "dfir_registry.DfirRegistry",     # registry key last-write times
]
...
def _container_argv(image, mem, plugin, symbols_dir):
...
def run_plugin(mem, plugin, out_dir, symbols_dir, *, image=DEFAULT_IMAGE, native=False) -> dict:
    ...
    argv = _native_argv(mem, plugin, symbols_dir) if native \
        else _container_argv(image, mem, plugin, symbols_dir)
...
def run_all(mem, out_dir, *, plugins=None, symbols_dir, image=DEFAULT_IMAGE, native=False) -> list[dict]:
    """Run the timeline + context plugins over one image. Returns per-plugin results."""
    plugins = plugins if plugins is not None else (TIMELINE_PLUGINS + CONTEXT_PLUGINS)
    return [run_plugin(mem, p, out_dir, symbols_dir, image=image, native=native) for p in plugins]
